...
class ServoControlSubscriber(Node):
    ...
    def listener_callback(self, msg):
        throttle_value = msg.throttle * 30  # Scale throttle value
        steering_angle = msg.angle * 30    # Scale angle for steering
...
def capture_stereo_camera_images(deepracer_id, T_left_final, T_right_final, img_w=640, img_h=480):
    ...
    T_left_camera_global = np.dot(T_base, T_left_final)
    T_right_camera_global = np.dot(T_base, T_right_final)

    left_img_data = capture_image(T_left_camera_global[:3, 3], T_left_camera_global[:3, 3] + np.dot(T_left_camera_global[:3, :3], np.array([1, 0, 0])), img_w, img_h)
    right_img_data = capture_image(T_right_camera_global[:3, 3], T_right_camera_global[:3, 3] + np.dot(T_right_camera_global[:3, :3], np.array([1, 0, 0])), img_w, img_h)
    
    left_img_rgb = np.array(left_img_data[2]).reshape((img_h, img_w, 4))[:, :, :3]
    right_img_rgb = np.array(right_img_data[2]).reshape((img_h, img_w, 4))[:, :, :3]
...
def display_images(left_img, right_img):
...
def main():
    ...
    deepracer_id = load_urdf("/home/do-gon/ros2_foxy_deepracer_ws/src/deepracer_description/meshes/3deepracer_stereo_cameras_and_lidar_urdf.urdf", [4.5, 0, 0.1], [0,0,(1/2)*np.pi])
    # object_id = load_static_urdf("/home/do-gon/ros2_foxy_deepracer_ws/src/pybullet-URDF-models/urdf_models/models/blue_moon/model.urdf", [4.0, 0, 0.1], [0, 0, 0.2])
    ...
    # curve90road1 = load_static_urdf("/home/do-gon/ros2_foxy_deepracer_ws/src/Models_downloaded_from_free3D/URDF_file/road_curve90.urdf", [2,2-0.007,0.27], [0, 0, (3/2)*np.pi])
    # curve90road2 = load_static_urdf("/home/do-gon/ros2_foxy_deepracer_ws/src/Models_downloaded_from_free3D/URDF_file/road_curve90.urdf", [2-0.007,-2,0.27], [0, 0, np.pi])
    # curve90road3 = load_static_urdf("/home/do-gon/ros2_foxy_deepracer_ws/src/Models_downloaded_from_free3D/URDF_file/road_curve90.urdf", [-2+0.07,2,0.27], [0, 0, (2)*np.pi])
    # curve90road4 = load_static_urdf("/home/do-gon/ros2_foxy_deepracer_ws/src/Models_downloaded_from_free3D/URDF_file/road_curve90.urdf", [-2,-2+0.07,0.27], [0, 0, np.pi/2])
    ...
    # grass = load_static_urdf("/home/do-gon/ros2_foxy_deepracer_ws/src/Models_downloaded_from_free3D/URDF_file/grass.urdf", [0,0,-0.23], [0, 0, np.pi/4])

    servo_control_subscriber = ServoControlSubscriber(deepracer_id)

    T_left_final = compute_transformation([0.091711, 0.03, 0.080023], [0, 0.2618, 0])
    T_left_final = np.dot(T_left_final, compute_transformation([0.044755, 0, 0.04], [0, 0, 0]))

    T_right_final = compute_transformation([0.091711, -0.03, 0.080023], [0, 0.2618, 0])
    T_right_final = np.dot(T_right_final, compute_transformation([0.044755, 0, 0.04], [0, 0, 0]))
    ...
    for step in range(100000):
        ...
        if step % 500 == 0:
            ## To get image data from two cameras
            # left_img, right_img = capture_stereo_camera_images(deepracer_id, T_left_final, T_right_final)
    ...
    # display_images(left_img, right_img)
